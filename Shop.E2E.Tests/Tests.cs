...
namespace Shop.Module.E2E.Tests;

public class ShopTests : IDisposable {
    const string BlazorAppName = "ShopBlazor";
    const string WinAppName = "ShopWin";
    const string AppDBName = "Shop";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public ShopTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\Shop.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\Shop.Win\bin\EasyTest\net8.0-windows\Shop.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "ShopEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
